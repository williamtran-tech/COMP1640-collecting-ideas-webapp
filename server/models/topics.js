...
Topic.getAllIdeasByTopic = (result, topicId) => {
    let query1 =`SELECT topic.id, topic, closure_date, final_closure_date, count(idea) as idea_quantity 
    FROM topic 
    INNER JOIN idea ON idea.topic_id = topic.id
    WHERE topic.id =` + topicId ;
    let query2 = `SELECT idea.id, idea.idea, idea.file_path, idea.created_at, category.category FROM idea
    ...
    sql.query(query,function (err, res) {
        if(err) {
            ...
            // console.log('Ideas by Topic: ', res);
            result(null, res);
        }
    });
}

// Topic.getTopicInfo = (result, topicId) => {
//     let query = `SELECT topic.id, topic, closure_date, final_closure_date, count(idea) as idea_quantity 
//     FROM topic 
//     INNER JOIN idea ON idea.topic_id = topic.id
//     WHERE topic.id = ` + topicId;
