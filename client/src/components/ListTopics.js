...
import { Box, CardHeader,Badge,IconButton, Chip, Typography, Stack } from '@mui/material'
  ...
  return (
    <Box>
        <Grid container  className='header' >
          ...
          {
            listtopics.departments?.map((department)=>(
              <>
              <Stack direction={"row"} spacing={1} className='list-department' ><BusinessIcon></BusinessIcon><Typography variant='h5' fontWeight={"bolder"}>{department.name}</Typography></Stack>
              
              <Grid container className='list-item'>
                { listtopics.topics?.filter(topic=> topic.Department.name  === department.name).map((topic) =>
